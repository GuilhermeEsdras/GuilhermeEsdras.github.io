...
btv.AbstractAlgorithm = function (tree, visualiser, isSubalgorithm) {
  ...
  this.treeCopy = btv.AbstractAlgorithm.copyTree(this.tree); // assistant variables; reset for new call
...
btv.AbstractAlgorithm.copyTree = function (tree) {
  ...
  if (tree.getRoot() !== null) {
    // not null => not an empty tree
    // copy of a root node
    treeCopy.setRoot(new btv.BinaryTreeNode(tree.getRoot().getValue())); // recursively copy the rest of tree - preorder
...
btv.AbstractAlgorithm.copyNodeRec = function (leftChildSource, rightChildSource, parentCopy) {
  if (leftChildSource != null) {
    // clone/copy
    parentCopy.setLeft(new btv.BinaryTreeNode(leftChildSource.getValue())); // recurse
  ...
  if (rightChildSource != null) {
    // clone/copy
    parentCopy.setRight(new btv.BinaryTreeNode(rightChildSource.getValue())); // recurse
...
btv.AbstractAlgorithm.prototype.redoStart = function () {
  this.redoCalls++; // fire start animation listeners
  ...
  if (this.treeCopy.selectedNode != null) {
    // select selected node
    // get index of selected node
    var selectedIndex = this.treeCopy.selectedNode.getIndex(); // get selected node

    var selectedNode = this.tree.getNode(selectedIndex); // if not selected select and show tree, othervise no selection nor showing is needed
    ...
    // this algorithm does not have a node as input value
...
btv.AbstractAlgorithm.prototype.undo = function () {
  ...
  var copy = btv.AbstractAlgorithm.copyTree(this.treeCopy); // set copy of treeCopy as the tree
...
btv.bst.RandomBSTreeAlg = function (tree, visualiser, min, max, isSubalgorithm) {
  ...
  this.max = max; // parent constructor
...
btv.bst.RandomBSTreeAlg.prototype.redo = function () {
  ...
  if (this.redoCalls > 1) {
    // repeated call - dont generate new random tree
  ...
  this.visualiser.redrawTree(this.tree); // routine
...
btv.bst.RandomBSTreeAlg.prototype.rec = function (node, level, min, max) {
  if (min >= node.getValue() - 1 || node.getValue() + 1 >= max) {
    // safety
    return;
  }
  ...
  if (level <= 3 && Math.random() < 0.5) {
    // make both child - generated tree is fuller
    chance = 1.0;
  } else {
    switch (level) {
      case 1:
...
btv.bst.InsertAlg = function (tree, visualiser, value, isSubalgorithm) {
   ...
   * @type {Number}
   */
  this.value = value; // parent constructor
...
btv.bst.InsertAlg.prototype.redo = function () {
  // routine
  this.redoStart(); // remove grapgic node - necessary if skip backward and the graphic was already added

  this.visualiser.animateRemoveNode(this.node);
  this.node = new btv.BinaryTreeNode(this.value); // show the new graphic node

  this.visualiser.animateShowNodeElemNextTo(this.node, 0);
  var node = this.tree.getRoot();

  if (node === null) {
    // root is null -> empty tree -> insert root
    // logic
    this.tree.setRoot(this.node); // move the graphic node
    // has no parent, not needed edgeElement

    this.visualiser.animateMoveTo(this.node, 0);
  } else {
    while (true) {
      if (this.node.getValue() >= node.getValue()) {
        // right
        ...
        if (node.getRight() === null) {
          // insert
          node.setRight(this.node); // move the graphic node

          this.visualiser.animateMoveTo(this.node, 2 * node.getIndex() + 2); // show the graphic node parent edge 

          this.visualiser.animateAddEdgeElem(this.node);
          break;
        } else {
          // next comparison
          node = node.getRight(); // move the graphic node
        ...
        if (node.getLeft() === null) {
          // insert
          node.setLeft(this.node); // move the graphic node

          this.visualiser.animateMoveTo(this.node, 2 * node.getIndex() + 1); // show the graphic node parent edge

          this.visualiser.animateAddEdgeElem(this.node);
          break;
        } else {
          // next comparison
          node = node.getLeft(); // move the graphic node
  ...
  this.visualiser.animateSelectNode(null); // routine
...
btv.bst.InsertAlg.prototype.toString = function () {
  return "inserir(valor: " + this.value + ")";
}; ////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////
...
btv.bst.FindAlg.prototype.redo = function () {
  ...
  if (node === null) {
    // root is null -> empty tree -> not found
...
btv.bst.FindAlg.prototype.toString = function () {
  return "buscar(valor: " + this.value + ")";
}; ////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////
...
btv.bst.DeleteAlg = function (tree, visualiser, node, isSubalgorithm) {
   ...
   */
  this.node = node; // copy of this reference will be created in the parent constructor - copy of given tree

  tree.selectedNode = this.node; // parent constructor
...
btv.bst.DeleteAlg.prototype.redo = function () {
  ...
  if (this.node.getLeft() !== null && this.node.getRight() !== null) {
    ...
    var successor = getSuccessorAlg.redo(); // swap this.node with successor
  ...
  if (this.node.getLeft() !== null) {
    // just left child
    var leftSubtreeRoot = this.node.getLeft(); // grapgic - delete
    ...
    var leftSubtreePreorderArray = btv.BinaryTree.toInorderArrayRec(leftSubtreeRoot); // grapgic - move subtree

    this.visualiser.animateMoveToIndexLoc(leftSubtreePreorderArray); // graphic - add edge

    this.visualiser.animateAddEdgeElem(leftSubtreeRoot, leftSubtreeRoot.getParent());
  } else if (this.node.getRight() !== null) {
    // just right child
    var rightSubtreeRoot = this.node.getRight(); // grapgic - delete
    ...
    var rightSubtreePreorderArray = btv.BinaryTree.toInorderArrayRec(rightSubtreeRoot); // grapgic - move subtree

    this.visualiser.animateMoveToIndexLoc(rightSubtreePreorderArray); // graphic - add edge
...
btv.bst.DeleteAlg.prototype.toString = function () {
  return "apagar(valor: " + this.treeCopy.selectedNode.getValue() + ", index: " + this.treeCopy.selectedNode.getIndex() + ")";
}; ////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////
...
btv.bst.GetMaxAlg.prototype.redo = function () {
  if (!this.isSubalgorithm) {
    ...
    // is called from another algorithm
    this.redoCalls++; // add animator that fire start animation listeners

    this.visualiser.animateStart(this); // select selected node
    // get index of selected node
  ...
  this.visualiser.animateAddAssistNodeElemNextTo(searchNode, this.node); // find the most right node - maximum

  node = this.node;

  if (node.getRight() === null) {
    // to show that I try ro go right
  ...
  this.visualiser.animateRemoveNode(searchNode); // select predecessor
...
btv.bst.GetMaxAlg.prototype.toString = function () {
  return "getMax(valor: " + this.treeCopy.selectedNode.getValue() + ", index: " + this.treeCopy.selectedNode.getIndex() + ")";
}; ////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////
...
btv.bst.GetMinAlg.prototype.redo = function () {
  ...
  this.visualiser.animateSelectNode(node); // this.redoEnd(!this.isSubalgorithm); 
...
btv.bst.GetMinAlg.prototype.toString = function () {
  return "getMin(valor: " + this.treeCopy.selectedNode.getValue() + ", index: " + this.treeCopy.selectedNode.getIndex() + ")";
}; ////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////
...
btv.bst.GetPredecessorAlg.prototype.redo = function () {
  ...
  if (node !== null) {
    ...
    this.visualiser.animateMoveNextTo(searchNode, node);
    this.visualiser.animateRemoveNode(searchNode); // use getMax algoritm as part of that algorithm

    var getMaxAlg = new btv.bst.GetMaxAlg(this.tree, this.visualiser, node, true);
    var returnedValue = getMaxAlg.redo(); // routine

    this.redoEnd();
    return returnedValue;
  } else {
    // no left subtree
    this.visualiser.animateShowChangeAssistNode(searchNode, String.fromCharCode(8665), true); // for first parent to that arrives from right
    ...
    while (true) {
      if (parent === null) {
        ...
        this.visualiser.animateShowChangeAssistNode(searchNode, String.fromCharCode(8657), true); // hide searchNode

        this.visualiser.animateShowRemoveNode(searchNode); // unselect all
      ...
      this.visualiser.animateShowChangeAssistNode(searchNode, String.fromCharCode(8657), false); // move nextTo parent if exists
      ...
      if (parent.getRight() === node) {
        // arrived from right
        // from right
        this.visualiser.animateShowChangeAssistNode(searchNode, String.fromCharCode(8598), false); // moveTo

        this.visualiser.animateMoveTo(searchNode, parent.getIndex());
        this.visualiser.animateRemoveNode(searchNode); // select predecessor

        this.visualiser.animateSelectNode(parent); // routine
...
btv.bst.GetPredecessorAlg.prototype.toString = function () {
  return "getPredecessor(valor: " + this.treeCopy.selectedNode.getValue() + ", index: " + this.treeCopy.selectedNode.getIndex() + ")";
}; ////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////
...
btv.bst.GetSuccessorAlg.prototype.redo = function () {
  ...
  if (node !== null) {
    ...
    // move next to left child
    this.visualiser.animateMoveNextTo(searchNode, node);
    this.visualiser.animateRemoveNode(searchNode); // use getMin algoritm as part of that algorithm

    var getMinAlg = new btv.bst.GetMinAlg(this.tree, this.visualiser, node, true); // and do it // add EndAnimator

    var returnedValue = getMinAlg.redo(); // routine

    this.redoEnd();
    return returnedValue;
  } else {
    // no rigt subtree
    this.visualiser.animateShowChangeAssistNode(searchNode, String.fromCharCode(8664), true); // for first parent to that arrives from left
    ...
    while (true) {
      ...
      if (parent.getLeft() === node) {
        // arrived from left
        // from left
        this.visualiser.animateShowChangeAssistNode(searchNode, String.fromCharCode(8599), false); // moveTo

        this.visualiser.animateMoveTo(searchNode, parent.getIndex());
        this.visualiser.animateRemoveNode(searchNode); // select successor
...
btv.bst.GetSuccessorAlg.prototype.toString = function () {
  return "getSuccessor(valor: " + this.treeCopy.selectedNode.getValue() + ", index: " + this.treeCopy.selectedNode.getIndex() + ")";
}; ////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////
...
btv.bst.ToPreorderArrayAlg = function (tree, visualiser, isSubalgorithm) {
   ...
   * @private
   */
  this.moveNode = null; // parent constructor
...
btv.bst.ToPreorderArrayAlg.prototype.redo = function () {
  var array = new Array(); // routine

  this.redoStart(); // add array element

  this.visualiser.animateAddArrayElem(this.tree.getCount()); // create moveNode
  ...
  if (node === null) {
    ...
    this.visualiser.animateShowChangeAssistNode(this.moveNode, String.fromCharCode(8659), true); // hide move node

    this.visualiser.animateShowRemoveNode(this.moveNode); // routine

    this.redoEnd();
    return array;
  }

  this.rec(node, array); // leave tree - change label

  this.visualiser.animateShowChangeAssistNode(this.moveNode, String.fromCharCode(8657)); // remove move node

  this.visualiser.animateRemoveNode(this.moveNode); // routine
...
btv.bst.ToPreorderArrayAlg.prototype.rec = function (node, array) {
  // change label
  this.visualiser.animateChangeAssistNode(this.moveNode, ""); // show index node

  var toArrayNode = new btv.BinaryTreeNode(node.getValue());
  toArrayNode.selectable = false;
  this.visualiser.animateShowNodeElemAt(toArrayNode, node); // move index node to arrayElem

  this.visualiser.animateMoveNextToArrayElem(toArrayNode); // hide index node

  this.visualiser.animateShowTree(0.25);
  this.visualiser.animateRemoveNode(toArrayNode); // show in array

  array.push(node);
  this.visualiser.animateShowInsertToArrayElem(node, array.length - 1);
  this.recLeft(node.getLeft(), array); // go up

  if (node.getLeft() !== null) {
    // only if has left child, otherwise moveNode is still here
    // change label
    this.visualiser.animateShowChangeAssistNode(this.moveNode, String.fromCharCode(8657)); // move

    this.visualiser.animateMoveNextTo(this.moveNode, node);
  }

  this.recRight(node.getRight(), array); // go up

  if (node.getRight() !== null) {
    // only if has right child, otherwise moveNode is still here
    // change label
    this.visualiser.animateShowChangeAssistNode(this.moveNode, String.fromCharCode(8657)); // move    
...
btv.bh.HeapifyUpAlg.prototype.rec = function (node) {
  ...
  if (difference >= 0) {
    ...
    // swap
    this.tree.swapNodes(this.node, parent); // graphic swap
...
btv.bh.HeapifyDownAlg.prototype.rec = function (node) {
  ...
  if (difference < 0) {
    ...
    this.visualiser.animateSwapNodeElems(node, greater); // recursion
...
btv.bh.RandomBHeapAlg.prototype.redo = function () {
  ...
  if (this.redoCalls > 1) {
    ...
    // first call
    var count = btv.getRandomInt(1, Math.min(this.max - this.min + 1, 31)); // generate array of random non identic numbers of scale
...
btv.bh.BuildHeapAlg = function (tree, visualiser, array, isSubalgorithm) {
   ...
   * @type {Number[]}
   */
  this.array = array; // parent constructor
...
btv.bh.BuildHeapAlg.prototype.redo = function () {
  ...
  this.visualiser.animateShowTree(); // build heap
...
btv.bh.InsertAlg.prototype.redo = function () {
  ...
  if (this.tree.getRoot() == null) {
    this.tree.setRoot(this.node); // show the new graphic node
  ...
  this.visualiser.animateShowSelectNode(this.node); // add node
...
btv.bh.DeleteAlg.prototype.redo = function () {
  ...
  if (length == 1) {
    this.tree.setRoot(null);
    this.visualiser.animateShowRemoveNode(this.node); // routine
  ...
  this.visualiser.animateShowRemoveNode(this.node); // heapify up or down (last) 
...
btv.bh.DeleteAlg.prototype.toString = function () {
  return "deletar(valor: " + this.treeCopy.selectedNode.getValue() + ", index: " + this.treeCopy.selectedNode.getIndex() + ")";
}; ////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////
...
btv.bh.ExtractMaxAlg.prototype.redo = function () {
  var max = this.tree.getRoot(); // routine
...
btv.bh.HeapSortAlg.prototype.redo = function () {
  var array = new Array();
  var length = this.tree.getCount(); // routine
  ...
  for (var i = length - 1; i >= 1; i--) {
    ...
    this.tree.setRoot(node); // graphic swap

    this.visualiser.animateSwapNodeElems(root, node); // add to array element
    // show assisstant node

    var assistant = new btv.BinaryTreeNode(root.getValue());
    assistant.selectable = false;
    this.visualiser.animateAddNodeElemAt(assistant, i); // remove root

    this.visualiser.animateRemoveNode(root); // move assistant node to arrayElem

    this.visualiser.animateMoveNextToArrayElem(assistant); // remove asistant

    this.visualiser.animateShowTree(0.25);
    this.visualiser.animateRemoveNode(assistant); // show in array

    this.visualiser.animateShowInsertToArrayElem(root, i);
    array.push(root); // heapify new root (node)
    // use heapify algoritm as part of that algorithm
  ...
  this.visualiser.animateAddNodeElemAt(last, 0); // remove root

  this.visualiser.animateRemoveNode(this.tree.getRoot()); // move assistant node to arrayElem

  this.visualiser.animateMoveNextToArrayElem(last); // remove root

  this.visualiser.animateShowTree(0.25);
  this.visualiser.animateRemoveNode(last); // show in array

  this.visualiser.animateShowInsertToArrayElem(this.tree.getRoot(), 0);
  array.push(this.tree.getRoot()); // back to heap and redraw;

  var copy = btv.AbstractAlgorithm.copyTree(this.treeCopy);
  this.tree.setRoot(copy.getRoot());
  this.visualiser.animateRedrawTree(this.tree); //this.visualiser.animateSelectNode(null);
  // routine
